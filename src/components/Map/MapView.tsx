...
export const MapView: React.FC = () => {
  ...
  const filteredSchools = mockSchools.filter(school => {
    if (
      ...
      !school.level.some(lvl => filters.type.includes(lvl))
    ) return false;
    ...
    if (
      school.students < filters.minStudents ||
      school.students > filters.maxStudents
    ) return false;
      ...
      <div className="flex-1 relative">
        <div className="h-full w-full">
          ...
          >
            ...
            />
            {filteredSchools.map((school) => {
              const primaryLevel = school.level[0];
              const schoolType = schoolLevels.find(t => t.value === primaryLevel);
        ...
        <div className="absolute top-6 right-6 flex flex-col space-y-3 z-[1000]">
          <button
            ...
            className={`p-4 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 ${showFilters ? 'bg-blue-50 text-blue-600 ring-2 ring-blue-200' : 'text-gray-600 hover:bg-gray-50'
              }`}
            title="Filtres"
          >
          ...
          <button
            ...
            className={`p-4 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 ${showList ? 'bg-blue-50 text-blue-600 ring-2 ring-blue-200' : 'text-gray-600 hover:bg-gray-50'
              }`}
        ...
        {/* Stats Card */}
        <div className="absolute top-1/2 left-6 bg-white p-6 rounded-xl shadow-lg z-[1000] border border-gray-200">
